...
api_key_status = "🟢 Active" if os.environ.get("GEMINI_API_KEY") else "🔴 Missing GEMINI_API_KEY"
...
with tab1:
    st.write("ربط مباشر بمستودع GitHub للتحليل الفوري.")
    repo_url = st.text_input("Enter Public GitHub URL:", placeholder="https://github.com/username/repository")
    
    if st.button("🚀 Fetch & Audit GitHub Repo", type="primary", key="btn_github"):
        if repo_url:
            try:
...
with tab2:
    ...
    if uploaded_file and st.button("🚀 Audit Local ZIP", type="primary", key="btn_zip"):
...
with tab3:
    ...
    if st.button("⚡ Quick Scan", type="primary", key="btn_snippet"):
...
if 'report' in st.session_state and st.session_state['report']:
    ...
    st.markdown(f"*Audit Summary:* {report.get('summary')}")
    ...
    for i, v in enumerate(vulns):
        ...
        with st.expander(f"{badge} [{severity}] {v.get('vulnerability_type')} — File: {v.get('file_path')}"):
            ...
            # تنفيذ الـ Unit Test المباشر
            test_key = f"run_test_{i}"
            if st.button(f"▶️ Execute Local Test Verification #{i+1}", key=test_key):
                with st.spinner("Executing Unit Test in temporary isolated Sandbox..."):
                    ...
                    if test_result["passed"]:
                        ...
                        st.error("❌ Test FAILED or Encountered Execution Issue:")
                        st.code(test_result["output"])